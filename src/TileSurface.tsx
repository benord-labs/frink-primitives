...
export function TileSurface({
  ...
  return (
    <div className={cn('tile-rim relative rounded-[22px] bg-bg p-3', className)} {...props}>
      <div className="relative overflow-hidden rounded-[14px]">
        ...
        {corner != null && (
          <div className="absolute top-3 right-4 z-10 font-mono text-xs text-muted">{corner}</div>
        )}
        <div className={cn('relative z-10 flex flex-col gap-6 px-6 pt-12 pb-6', contentClassName)}>
          {children}
        </div>
      </div>
    </div>
