 ...
 */
export function Tile({ title, meta, year, href, media, className, ...props }: TileProps) {
  const cardClass = cn(
  ...
  const inner = (
    <div className="relative aspect-video w-full overflow-hidden rounded-[14px] bg-bg">
      ...
      />
      {year != null && (
        <span className="absolute top-2.5 right-4 font-mono text-xs text-muted">{year}</span>
      )}
  ...
  if (href) {
    return (
      <a href={href} className={cardClass} {...(props as HTMLAttributes<HTMLAnchorElement>)}>
        {inner}
      </a>
    );
  }
