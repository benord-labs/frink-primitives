...
function detectPerformancePatterns(files, diffs) {
	...
	if (
		RE_IMAGE_DIFF.test(fullDiff) ||
		files.some((f) => RE_IMAGE_FILE.test(f))
	) {
		items.push({
...
function generate() {
	...
	saveChecklist(data);
	log(
		`✅ Frontend Performance: ${stagedFiles.length} files, ${items.length} checks`,
	);
	log("");
	log("Items to review:");
