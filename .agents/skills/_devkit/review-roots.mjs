...
export function normalizeReviewRoots(value, name) {
	if (!Array.isArray(value) || value.length === 0)
		throw new Error(
			`${name} must be a non-empty JSON array of repository-relative paths`,
		);
...
export function resolveConfigRoots({ configKey, reviewerName }) {
	...
	if (roots === undefined || (Array.isArray(roots) && roots.length === 0))
		return ["."];
	try {
...
export function resolveReviewRoots({ envName, configKey, reviewerName }) {
	...
	const roots =
		review && typeof review === "object" ? review[configKey] : undefined;
