...
const RE_INPUT =
	/\b(input|onChange|onSubmit|formData|useForm)|\bvalue\s*=\s*[{"']/i;
const RE_BLANK = /target\s*=\s*["']_blank/i;
const RE_WINDOW_OPEN = /window\.open/i;
const RE_EVAL =
	/\b(eval\s*\(|new\s+Function\s*\(|setTimeout\s*\(\s*["']|setInterval\s*\(\s*["'])/i;
const RE_URL_PARAMS =
	/\b(window\.location|searchParams|URLSearchParams|queryString)/i;
...
const RE_HARDCODED =
	/\b(users|pass|key|secret|token)\s*[:=]\s*["'][^"']{8,}["']/i;
...
function getStagedFiles() {
	...
	if (override)
		return override.filter(
			(f) => !f.endsWith(".pen") && !RE_PROSE_FILE.test(f),
		);
...
function generate() {
	...
	saveChecklist(data);
	log(
		`✅ Frontend Security: ${stagedFiles.length} files, ${items.length} checks`,
	);
	log("");
	log("Items to review:");
