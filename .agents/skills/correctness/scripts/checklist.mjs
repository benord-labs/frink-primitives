...
function extractLens(argv) {
	...
	if (lenses.length === 0 || unknown.length > 0) {
		console.error(
			`❌ --lens must be a comma-separated subset of: ${known.join(", ")}`,
		);
		process.exit(1);
	}
...
function unionRoots() {
	...
	const injectedBackend = parseInjectedReviewRoots(
		"DEVKIT_REVIEW_BACKEND_ROOTS",
	);
	const injectedFrontend = parseInjectedReviewRoots(
	...
	const review =
		typeof c.review === "object" && c.review !== null ? c.review : {};
...
function getStagedFiles() {
	...
	if (override) {
		const exts = sourceExtensions().map(
			(e) => `.${e.replace(RE_LEADING_DOT, "")}`,
		);
		return override.filter(
			(f) => exts.some((x) => f.endsWith(x)) && !RE_TEST_INFIX.test(f),
		);
	}
	...
	try {
		...
		console.error(
			`❌ correctness: \`git diff --cached\` failed — ${e.message ?? e}`,
		);
		process.exit(1);
	}
}
...
const ALL_ITEMS = [
	...
	{
		name: "writer-reader-contracts",
		category: "Contract, Boundary & Broadcast",
	},
	{ name: "error-and-edge-classification", category: "Classifier & Parsing" },
];

function detectCorrectnessItems(lens) {
	return ALL_ITEMS.filter((it) => !lens || lens.includes(it.name)).map(
		(it) => ({
			...it,
			status: "pending",
			issues: [],
...
function generate(lens) {
	...
	if (stagedFiles.length === 0) {
		log(
			"⏭️  No staged source files under the declared roots. Skipping correctness review.",
		);
...
switch (cmd) {
	...
	case "check-item": {
		...
		if (!name || (!pass && failIdx === -1)) {
			log(
				'Usage: check-item [--lens <a[,b]>] <name> --pass OR --fail "reason"',
			);
			process.exit(1);
		}
	...
	default:
		log(
			"Correctness Review Commands (all accept --lens <a[,b,...]> to scope to a lens group):",
		);
