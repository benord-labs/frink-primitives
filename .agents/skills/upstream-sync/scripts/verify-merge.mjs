...
function loadAcceptedFiles() {
	...
	try {
		...
		if (VERBOSE) {
			const rejected = review.files.filter(
				(f) => f.status === "rejected",
			).length;
...
function main() {
	...
	if (acceptedFiles !== null) {
		...
		if (VERBOSE && before !== stagedFiles.length) {
			log(
				`   Filtered to ${stagedFiles.length}/${before} staged files (accepted only)\n`,
			);
		}
	}
	...
	for (const file of stagedFiles) {
		...
		const significantAdditions = frinkAdditions.filter((a) =>
			isSignificantLine(a.trimmed),
		);
		...
		if (missing.length > 0) {
			...
			const extraMsg =
				extra > 0
					? ` (${modified.length} modified, ${movedToOtherFile.length} moved)`
					: "";
			log(
				`  ✅ ${file} — ${significantAdditions.length} frink additions preserved${extraMsg}`,
			);
		}
	}
	...
	log("─".repeat(50));
	log(
		`📊 Results: ${totalChecked} frink additions checked across ${stagedFiles.length} files`,
	);
