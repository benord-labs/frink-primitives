...
// Form inputs
const RE_INPUT =
	/<input\b|<select\b|<textarea\b|<Input\b|<Select\b|<Textarea\b/i;
...
// Focus handling
const RE_FOCUS =
	/\bonFocus\b|\bonBlur\b|:focus|focus-visible|focus:ring|\.focus\(\)/i;

// Color & styling (potential contrast issues)
const RE_COLOR =
	/\bcolor\s*:|bg-|text-(?!xs|sm|base|lg|xl|2xl|3xl)|fill\s*=|stroke\s*=/i;

// Motion & animation
const RE_MOTION =
	/\banimation\b|\btransition\b|@keyframes|\bmotion\b|prefers-reduced-motion/i;
...
const RE_DYNAMIC =
	/\buseState\b|\buseEffect\b|\binnerHTML\b|\baria-live\b|\brole\s*=\s*["']alert/i;
...
const RE_STATUS_STATE =
	/\berror\b|\bwarning\b|\bsuccess\b|\binvalid\b|\balert\b/i;
...
function getStagedFiles() {
	...
	try {
		...
		if (!output.trim())
			assertStagedSetSane(pathspecs, "frontend-accessibility");
		return output
			.trim()
			.split("\n")
...
function detectAccessibilityPatterns(_files, diffs) {
	...
	if (RE_HEADING.test(fullDiff)) {
		items.push({
			...
			description:
				"Headings used in logical order, no skipped levels, one h1 per page",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_LINK.test(fullDiff)) {
		items.push({
			...
			category: "Interactive Controls",
			description:
				'Links use <a> with href; external links have rel="noopener noreferrer"',
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_BLANK.test(fullDiff)) {
		items.push({
			...
			category: "Interactive Controls",
			description:
				'External links indicate new tab and use rel="noopener noreferrer"',
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_ARIA.test(fullDiff)) {
		items.push({
			...
			description:
				"ARIA used correctly — no redundant roles, valid attributes, proper labeling",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_OUTLINE_NONE.test(fullDiff)) {
		items.push({
			...
			category: "Keyboard & Navigation",
			description:
				"No outline:none without replacement focus style (focus-visible preferred)",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_COLOR.test(fullDiff)) {
		items.push({
			...
			description:
				"Text contrast >= 4.5:1 (normal) / 3:1 (large); color not sole indicator",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_MOTION.test(fullDiff)) {
		items.push({
			...
			description:
				"Animations respect prefers-reduced-motion; no 3+ flashes/sec",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_AUTOFOCUS.test(fullDiff)) {
		items.push({
			...
			category: "Keyboard & Navigation",
			description:
				"Avoid autofocus — disorients screen reader and motor-impaired users",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_LANG.test(fullDiff)) {
		items.push({
			...
			category: "Structure & Semantics",
			description:
				"lang attribute present on html element for correct pronunciation",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_DIALOG.test(fullDiff)) {
		items.push({
			...
			category: "ARIA & Custom Widgets",
			description:
				"Dialogs trap focus correctly, have aria-label, return focus on close",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_TOOLTIP.test(fullDiff)) {
		items.push({
			...
			category: "ARIA & Custom Widgets",
			description:
				"Avoid title attribute tooltips; use accessible alternatives",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_KEYBOARD.test(fullDiff)) {
		items.push({
			...
			category: "Keyboard & Navigation",
			description:
				"Keyboard handlers include appropriate key bindings; no keyboard traps",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_SCROLL.test(fullDiff)) {
		items.push({
			...
			description:
				"Scrollable areas are keyboard accessible; no horizontal scroll at 320px",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_LANDMARK.test(fullDiff)) {
		items.push({
			...
			category: "Structure & Semantics",
			description:
				"Landmark elements (main, nav, header, footer, aside) used for content regions",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_SKIP_LINK.test(fullDiff)) {
		items.push({
			...
			category: "Keyboard & Navigation",
			description:
				"Skip link present and visible when focused, targets main content",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_STATUS_STATE.test(fullDiff)) {
		items.push({
			...
			category: "Color & Contrast",
			description:
				"Error, warning, and success states use icons/text in addition to color",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_ORIENTATION.test(fullDiff)) {
		items.push({
			...
			category: "Responsive & Touch",
			description:
				"No CSS orientation lock — site can rotate to any orientation",
			status: "pending",
			issues: [],
		});
	}
	...
	if (RE_INPUT.test(fullDiff) && RE_COLOR.test(fullDiff)) {
		items.push({
			...
			category: "Color & Contrast",
			description:
				"Input borders have >= 3:1 contrast ratio against adjacent colors",
			status: "pending",
			issues: [],
		});
	}
...
function generate() {
	...
	saveChecklist(data);
	log(
		`✅ Frontend Accessibility: ${stagedFiles.length} files, ${items.length} checks`,
	);
	log("");
	log("Items to review:");
	for (const item of items)
		log(`  - [${item.category}] ${item.name}: ${item.description}`);
}
