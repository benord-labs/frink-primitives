...
const RE_AUTH =
	/\b(auth|login|signin|signup|password|credential|encryption|encrypt|decrypt|hash)/i;
const RE_JWT =
	/\b(jwt|jsonwebtoken|token|sign|verify|bearer|expiresIn|algorithm)/i;
...
const RE_OAUTH =
	/\b(oauth|redirect_uri|client_id|client_secret|response_type)/i;
...
const RE_ENDPOINT =
	/\b(router|handler|endpoint|route|get|post|put|patch|delete)\s*[.(]/i;
const RE_RATE_LIMIT = /\b(rate|limit|throttle|rateLimit)/i;
const RE_RESPONSE = /\b(res\.|response\.|json\(|send\(|return.*Response)/i;
const RE_HEADERS =
	/\b(header|X-Content-Type|X-Frame|Content-Security-Policy|HSTS|X-Powered-By)/i;
...
const RE_COMMAND =
	/\b(exec|execSync|spawn|spawnSync|execFile|execFileSync)\s*\(|child_process/;
const RE_FS_SINK =
...
const RE_OUTBOUND_DYNAMIC =
	/\b(fetch|axios[.\w]*|got)\s*\(\s*(`[^`\n]*\$\{|[^'"`\s)])/;
...
function getStagedFiles() {
	...
	try {
		...
		if (!output.trim()) assertStagedSetSane(pathspecs, "api-security");
		return output
			.trim()
			.split("\n")
			.filter((f) => f.length > 0)
