...
function getOneCodeDir() {
	...
	if (existsSync(join("cloned-projects", "1code", ".git")))
		return join("cloned-projects", "1code");
	return "1code";
}
...
function getPendingCommits(state) {
	...
	const result = exec(
		`git -C ${ONECODE_DIR} log --oneline --reverse ${lastHash}..HEAD`,
		{
			allowFail: true,
		},
	);
...
function status() {
	...
	const totalDeferred = partialCommits.reduce(
		(sum, c) => sum + (c.deferred?.length ?? 0),
		0,
	);
...
function show(hash) {
	...
	const files = exec(
		`git -C ${ONECODE_DIR} diff --name-only ${hash}~1..${hash} ${fileFilter}`,
	);
	log(`Files changed:\n${files}\n`);

	// Show stat (filtered if chunked)
	const stat = exec(
		`git -C ${ONECODE_DIR} diff --stat ${hash}~1..${hash} ${fileFilter}`,
	);
	log(`Stats:\n${stat}\n`);
	...
	log("Full diff:\n");
	const diff = exec(
		`git -C ${ONECODE_DIR} diff ${hash}~1..${hash} ${fileFilter}`,
	);
	log(diff);
}
...
function accept(hash) {
	...
	if (useReviewFile) {
		...
		if (reviewHash !== fullHash) {
			log(
				`❌ Review file is for commit ${review.commit}, not ${hash}. Run review-clean first.`,
			);
			process.exit(1);
		}
		...
		const accepted = review.files
			.filter((f) => f.status === "accepted")
			.map((f) => f.path);
		const rejected = review.files
			.filter((f) => f.status === "rejected")
			.map((f) => f.path);
		const deferred = review.files
		...
		if (isHunkHeader) {
			...
			if (hunkMatch) {
				...
				keepHunk = ranges.some((r) => {
					...
					return (
						hunkOldStart <= rEnd + margin && hunkEnd >= r.oldStart - margin
					);
				});
			} else {
				keepHunk = true;
...
function frinkDelta(hash) {
	...
	const filesRaw = exec(
		`git -C ${ONECODE_DIR} diff --name-only ${hash}~1..${hash}`,
	);
	if (!filesRaw) {
	...
	const fileFilter = chunkFileSet
		? `-- ${[...chunkFileSet].map((f) => `"${f}"`).join(" ")}`
		: "";
	const upstreamDiff = exec(
		`git -C ${ONECODE_DIR} diff ${hash}~1..${hash} ${fileFilter}`,
	);
	...
	for (const file of files) {
		...
		if (!frinkExists) {
			log(
				"  ❌ File exists in 1code baseline but NOT in frink (deleted or moved)\n",
			);
			continue;
		}

		// Compute full diff between 1code baseline and frink's current version
		const delta = exec(`diff -u "${onecodePath}" "${frinkPath}" || true`, {
			allowFail: true,
		});
		...
		if (!delta) {
			...
			const totalAdded = allLines.filter(
				(l) => l.startsWith("+") && !l.startsWith("+++"),
			).length;
			const totalRemoved = allLines.filter(
				(l) => l.startsWith("-") && !l.startsWith("---"),
			).length;
			...
			const shownAdded = filteredContentLines.filter((l) =>
				l.startsWith("+"),
			).length;
			const shownRemoved = filteredContentLines.filter((l) =>
				l.startsWith("-"),
			).length;

			log(
				`  Frink total delta: +${totalAdded} -${totalRemoved} lines from baseline`,
			);
			...
			if (filteredLines.length <= MAX_LINES_PER_FILE) {
				...
				log(
					`  ... (${filteredLines.length - MAX_LINES_PER_FILE} lines omitted) ...`,
				);
...
function checkSize(hash) {
	...
	const numstatRaw = exec(
		`git -C ${ONECODE_DIR} diff --no-renames --numstat ${hash}~1..${hash}`,
	);
	if (!numstatRaw) {
	...
	for (const line of lines) {
		...
		totalLines +=
			(add === "-" ? 0 : Number(add)) + (del === "-" ? 0 : Number(del));
	}

	const isLarge =
		totalLines >= SIZE_THRESHOLD_LINES || filesCount >= SIZE_THRESHOLD_FILES;
...
function reviewInit(hash) {
	...
	if (existsSync(REVIEW_PATH)) {
		...
		// fallow-ignore-next-line code-duplication
		log(
			"⚠️  Review file already exists. Run review-clean first to start fresh.",
		);
		process.exit(1);
	}
	...
	for (const f of fileStats) {
		...
		if (
			...
			currentChunk.lines + fileLines > budget
		) {
	...
	log(
		`📋 Pre-populated ${files.length} files in ${chunks.length} chunk(s) (budget: ${budget})`,
	);
	log("─".repeat(60));
	for (const chunk of chunks) {
		const overBudget = chunk.files.length === 1 && chunk.lines > budget * 2;
		const warning = overBudget
			? " ⚠️  exceeds 2x budget — may overwhelm reviewer context"
			: "";
		log(
			`  Chunk ${chunk.id}: ${chunk.files.length} files, ${chunk.lines} lines${warning}`,
		);
	}
...
function logFileEntry(f) {
	...
	log(
		`   [${f.id}] ${f.path} (+${f.additions}/-${f.deletions})${chunkTag} — ${label}`,
	);
	if (f.context) {
		log(`       → ${f.context}`);
	}
...
function parseIds(idsArg) {
	if (!idsArg) {
		log(
			"Usage: review-accept <ids> or review-reject <ids> (comma-separated, 1-based)",
		);
		process.exit(1);
	}
	const raw = [
...
function reviewSetStatus(idsArg, targetStatus) {
	if (!REVIEW_ALLOWED_STATUSES.has(targetStatus)) {
		log(
			`❌ Invalid status "${targetStatus}". Use: accepted, rejected, or deferred.`,
		);
		process.exit(1);
	}
	const review = loadReview();
...
function reviewMergeChunks() {
	...
	for (const tempFile of tempFiles) {
		...
		for (const entry of tempData.files) {
			...
			if (!reviewFile) {
				errors.push(
					`${tempFile}: path "${entry.path}" not found in review JSON`,
				);
				continue;
			}
			...
			if (entry.conflictRisk !== undefined)
				reviewFile.conflictRisk = entry.conflictRisk;
	...
	const incomplete = review.files.filter(
		(f) => f.summary === null || f.context === null,
	);
	if (incomplete.length > 0) {
		...
		log(
			`⚠️  ${incomplete.length} file(s) still have null summary/context after merge:`,
		);
...
function reviewWorkgroups() {
	...
	for (const f of accepted) {
		const isShared =
			SHARED_PATH_PREFIXES.some((prefix) => f.path.startsWith(prefix)) ||
			!f.path.includes("/"); // root config files
		if (isShared) {
			shared.push(f);
		} else {
			remaining.push(f);
	...
	for (const wg of workgroups) {
		log(
			`  Workgroup ${wg.id} (${wg.name}): ${wg.files.length} files, ${wg.lines} lines`,
		);
	}
...
function reset() {
	...
	log(
		"🔄 Sync state reset. Hash preserved at:",
		state.lastSyncedHash.slice(0, 8),
	);
}
...
switch (cmd) {
	...
	default:
		...
		log(
			"  show <hash> [--chunk N]   Full diff for a commit (optionally filtered by chunk)",
		);
		log(
			"  frink-delta <hash> [--chunk N]  Frink vs baseline diffs (optionally by chunk)",
		);
		log(
			"  check-size <hash>         Check commit size → Path A (standard) or Path B (chunked)",
		);
		log(
			"  accept <hash>             Accept and advance (--review-file for partial)",
		);
		log("  skip <hash>               Skip and advance");
		log(
			"  review-init <hash>        Pre-populate review JSON with files and chunks",
		);
		log(
			"                            Use --budget N to set lines-per-chunk (default 950)",
		);
		...
		log(
			"  review-clean              Remove review + chunk temp + workgroup files",
		);
